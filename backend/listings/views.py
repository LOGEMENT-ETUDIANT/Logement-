...
class ListingViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def stats(self, request):
        ...
        global_qs = Listing.objects.filter(price__isnull=False, surface__isnull=False)
        ...
        zone_qs = (
            ...
            .annotate(count=Count('*'), avg_price=Avg('price'), avg_surface=Avg('surface'))
            .order_by('-count')
        )
        ...
        current_count = agg['total'] or 0
        finding_probability = 0
        if max_zone_count > 0 and current_count > 0:
            finding_probability = round(min((current_count / max_zone_count) * 100, 100), 1)
